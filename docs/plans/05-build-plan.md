...
- Repo scaffolding per ADR-009: [`spo-assistants-api`](https://github.com/brianpmccullough/spo-assistants-api)
  (incl. `/docs` — this set) and
  [`spo-assistants-spfx`](https://github.com/brianpmccullough/spo-assistants-spfx).
  Own `.nvmrc` + lockfile + CI each; each defines its own model copies per doc 03.
